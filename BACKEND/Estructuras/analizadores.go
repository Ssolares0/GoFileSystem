...
func CreateNewDisk(size_int int, unit string, fit string) {
	...
	if _, err := os.Stat("./MIA"); os.IsNotExist(err) {
		err = os.Mkdir("./MIA", 0777)
		if err != nil {
	...
	archivos, err := ioutil.ReadDir("./MIA")
	if err != nil {
	...
	file, err := os.Create("./MIA/" + nameDisk + ".dsk")
...
func CreateFdisk(size_int int, unit string, fit string, drive string, name string, type_ string, delete string, add string, add_flag bool, delete_flag bool) {
	...
	//Abrimos el disco
	file, err := os.OpenFile("./MIA/"+drive+".dsk", os.O_RDWR, 0777)
	if err != nil {
...
func UnMount(id string) {
	...
	//Abrimos el disco
	file, err := os.OpenFile("./MIA/"+string(primerCaracter)+".dsk", os.O_RDWR, 0777)
	if err != nil {
		fmt.Println("Error al abrir el disco: ", err)
		return
	}
...
func VerificarPartMontada(id string) int {
	//buscamos el id en la lista
	for i := 0; i < len(MountList); i++ {
		if string(MountList[i].ID_part[:]) == id {
			return i
		}
	}
	return -1
