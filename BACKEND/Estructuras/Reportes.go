...
func ReporteJournaling(id string, path string, name string) {
	...
	if sb.SFilesystemType != 3 {
		fmt.Println("Error: El sistema de archivos no es ext3")
		return
	}
	_, err = archivo.Seek(MountActual.Start_part+int64(binary.Size(Superblock{})), 0)
	if err != nil {
	...
	Dot := ("digraph G{\n")
	Dot += ("a0[shape=none, color=lightblue2, style=filled, label=<\n<TABLE cellspacing=\"3\" cellpadding=\"2\" border=\"0\" cellborder=\"1\" bgcolor=\"#f0f8ff\">\n")
	Dot += ("<TR><TD colspan=\"4\" align=\"center\" bgcolor=\"#6495ED\"><FONT COLOR=\"#FFFFFF\">Reporte Journaling</FONT></TD></TR>\n")
	Dot += ("<TR><TD bgcolor=\"#ADD8E6\">Operacion</TD><TD bgcolor=\"#ADD8E6\">Path</TD><TD bgcolor=\"#ADD8E6\">Contenido</TD><TD bgcolor=\"#ADD8E6\">Fecha</TD></TR>\n")

	for i, cont := range Journaling.Journal {

		if i >= int(Journaling.Journal_last) {
			break
		}
		OperacionJournaling := RemoverNulos(string(cont.Tipo_operacion[:]))
		PathJournaling := RemoverNulos(string(cont.Path[:]))
		ContenidoJournaling := RemoverNulos(string(cont.Contenido[:]))
		TimeJournaling := RemoverNulos(string(cont.Time[:]))

		timeFormat := "2006-01-02 15:04:05"
		t, err := time.Parse(timeFormat, TimeJournaling)
		if err != nil {
			fmt.Println("Error al convertir la fecha: ", err)
			return
		}
		stringfecha := t.Format("2006-01-02 15:04:05")
		Dot += fmt.Sprintf("<TR><TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%s</TD></TR>\n", OperacionJournaling, PathJournaling, ContenidoJournaling, stringfecha)
